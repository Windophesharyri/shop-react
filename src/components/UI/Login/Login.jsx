...
function Login({state}) {
    ...
    const onClickRegistration = () => { setRegistration(current => !current); state(current => !current) }

    const onClickClose = () => { state(current => !current) }

    const onClickLogin = () => { 
        if (localStorage.getItem('authoritorized') == null || localStorage.getItem('authoritorized') == false) {
        ...
        <div className={cl.root}>
            <div className={cl.rootModal}>
                <div className={cl.inner}>
                    ...
                    <p className={cl.mainP}>Вход в личный кабинет</p>
                    <div>
                        <input className={cl.firstInput} placeholder="Электронная почта"></input>
                        <input className={cl.secondInput} placeholder="Пароль"></input>
                        <p className = {cl.passwordReminder}>Не помню пароль</p>
                    </div>
                    <div className = {cl.buttons}>
                        <button className = {cl.login} onClick = {onClickLogin}>Войти в кабинет</button>
                        <button className = {cl.registration} onClick = {onClickRegistration}>Регистрация</button>
